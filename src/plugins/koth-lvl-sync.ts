...
export default definePlugin({
  name: 'levelSync',
  description: 'Синхронизация уровней KOTH в cfg префиксов.',
  optionsSchema,
  setup({ state, options, logger, registerDisposable }) {
    ...
    const getRankImageByTotalXP = (totalXP: number): string => {
      ...
      for (let i = RANK_LEVELS.length - 1; i >= 0; i--) {
        if (levelFromTotalXP >= RANK_LEVELS[i]) return IMAGE_URLS[i];
      }
      return IMAGE_URLS[0];
    };
    ...
    const onPlayerConnected = async (data: TPlayerConnected) => {
      ...
      await serialize(() => updatePlayerLevel(steamID, eosID));
    };
    ...
    const onRoundEnded = async () => {
      ...
      for (const player of players) {
        ...
        await serialize(() => updatePlayerLevel(steamID, user.eosID));
      }
    };
